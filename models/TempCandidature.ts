...
export interface ITempCandidature extends Document {
    orderId: string;
    depositId: string;
    personalInfo: {
        fullName: string;
...
const TempCandidatureSchema: Schema = new Schema(
    {
...
// Expire after 24 hours if not moved
TempCandidatureSchema.index({ createdAt: 1 }, { expireAfterSeconds: 86400 });

// Prevent overwriting model during hot reload
const TempCandidature: Model<ITempCandidature> =
    mongoose.models.TempCandidature || mongoose.model<ITempCandidature>('TempCandidature', TempCandidatureSchema);

export default TempCandidature;
